...
class WeatherData:
    def __init__(self, dataset: xr.Dataset, window_size: int = 24):
    ...
    def weather_gifs(self, ds_, ds_f = None, feature='wspd', metric='m/s', levels=20, frames=0, frame_rate=16):
        ...
        ani.save('weather.gif', writer='pillow', fps=frame_rate)

        plt.close(fig)

        return HTML(ani.to_jshtml())

    def plot_window_target(self, seed = 0, frame_rate=16, save = False):
        ...
        tar = axs[1].contourf(self.dataset.longitude, self.dataset.latitude, targets, levels=20, vmin=vmin, vmax = vmax)
        ...
        if save:
            ani.save('weather_gif.gif', writer='pillow', fps=frame_rate)
